...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--data_path', default='skateboard/Tricks', help="Path to dataset")
    parser.add_argument(
        '--out_folder', default='skateboard/Tricks_openpose_result', help="Path to save files")
    parser.add_argument(
        '--model_folder', default='/openpose/models',help="Path to model folder")
    arg = parser.parse_args()
    arg.trainfile=os.path.join(arg.data_path,"label_train_tricks.csv")
    arg.testfile=os.path.join(arg.data_path,"label_val_tricks.csv")
    openpose='{}/examples/openpose/openpose.bin'.format(arg.openpose)
    LOGGER.info(os.getcwd())
    part = ['label_train_skating63', 'label_val_skating63']
    restart_list = {
        'label_train_tricks': 0,
        'label_val_tricks': 0,
    }
    debug = False
    debug_count = 2
